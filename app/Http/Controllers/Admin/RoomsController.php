...
class RoomsController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $cleansBy_list = \DB::select("SELECT `cleans`.`date`,`cleans`.`type`,`cleans`.`user_id`,`cleans`.`room_id`,`cleans`.`id`,`cleans`.`remarks`,`cleans`.`created_at`,`users`.`name`,`rooms`.`room_number` FROM `cleans`,`users`,`rooms` WHERE cleans.user_id=users.id AND `cleans`.`room_id`=`rooms`.`id` AND `cleans`.`room_id`=$id;");
